...
const getBrigades = () => {
	store.dispatch('brigades/getBrigades', params.value)
}
...
<template>
	<v-row>
		<v-col>
			...
			<app-table :loading="loadingBrigades"
					   :data="brigades"
					   :exclude="['brigade_id']"
			>
				<template #head:department_id>
					Department
				</template>
				<template #head:name>
					Brigade
				...
				<template #cell:department_id="{ row }">
					{{ departments.find(department => department.department_id === row.department_id)?.name }}
				</template>
			</app-table>
		</v-col>
	</v-row>
